...
public class Main {
	...
	Scanner scanner = new Scanner(System.in);
        ...
        byte[] text = DatatypeConverter.parseHexBinary(source);
        int[] fourBlocks = new int[4];
        for(int i = 0; i < 4; i++) {
            fourBlocks[i] = text[4 * i] + 256 * text[4 * i + 1] + (text[4 * i + 2] << 16) + (text[4 * i + 3] << 24);
        }

        System.out.println("Input:");
        printInput(fourBlocks);
        int[] key = fourBlocks;
        System.out.println("Key:");
        printInput(key);

        int[] encrypted = encrypt(fourBlocks, key);
    ...
    static int[] encrypt(int[] plainText, int[] key) {
        ...
        int[] whitened = doWhitening(plainText, roundKey01[0], roundKey01[1], roundKey23[0], roundKey23[1]);
        for (int i = 0; i < 16; i++) {
            whitened = encryptionRound(whitened, key, i);
            whitened = new int[] {whitened[2], whitened[3], whitened[0], whitened[1]};
        }
        whitened = new int[] {whitened[2], whitened[3], whitened[0], whitened[1]};
        whitened = doWhitening(whitened, roundKey45[0], roundKey45[1], roundKey67[0], roundKey67[1]);
    ...
    public static int[] decrypt(int[] cypheredText, int[] key) {
        ...
        final int[] roundKey45 = roundKeys(key, 2);
        final int[] roundKey67 = roundKeys(key, 3);
        int[] whitened = doWhitening(cypheredText, roundKey45[0], roundKey45[1], roundKey67[0], roundKey67[1]);
        whitened = new int[] {whitened[2], whitened[3], whitened[0], whitened[1]};
        for(int i = 15; i >= 0; i--) {
            whitened = decryptionRound(whitened, key, i);
            whitened = new int[] {whitened[2], whitened[3], whitened[0], whitened[1]};
        }
        whitened = doWhitening(whitened, roundKey01[0], roundKey01[1], roundKey23[0], roundKey23[1]);

        return whitened;
    }

    public static int[] doWhitening(int[] plainText, int k0, int k1, int k2, int k3) {
        return new int[] {
                plainText[0] ^ k0,
                plainText[1] ^ k1,
                plainText[2] ^ k2,
    ...
    private static byte[] multiply(Galua256 galua, byte[][] matrix, byte[] vector) {
        ...
        for(int i = 0; i < matrix.length; i++) {
            ...
            S[i] = galua.multi(RSrow[0], vector[0]);
            for(int j = 1; j < RSrow.length; j++) {
                S[i] = galua.add(S[i], galua.multi(RSrow[j], vector[j]));
            }
        }
